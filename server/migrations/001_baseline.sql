-- 001_baseline.sql — FROZEN. Do not edit this file.
--
-- This is the schema as it stood when the migration ledger was introduced. Its
-- SHA-256 is recorded in server/lib/migrations.js and in every database's
-- schema_migrations row; changing a byte of it makes every existing deployment
-- fail its checksum check, which is the entire point.
--
-- It lived inside db.js as a template literal that every feature phase appended
-- to. That is fine for a bootstrap and useless as a historical migration: a
-- checksum over a file somebody keeps editing records only that it changed
-- again. So the content is frozen here, and db.init() reads it rather than
-- carrying it.
--
-- **Future schema changes go in 002_*.sql, 003_*.sql and so on.** Nothing may
-- be added to this file, including a "small" column. If you find yourself
-- wanting to, the answer is a new numbered migration.
--
-- Idempotent by construction (CREATE TABLE IF NOT EXISTS, ADD COLUMN IF NOT
-- EXISTS, DO $$ blocks that check before they alter), so it is safe to run
-- against a database that already has some or all of it — which is what makes
-- the "upgrade an older database" path in scripts/migrate.js possible.
