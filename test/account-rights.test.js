...
after(async () => {
  if (created.users.length) {
    // privacy_request_events is deliberately NOT cleaned up: the table refuses
    // DELETE, which is the property test 26 exists to prove. The rows outlive
    // the fabricated accounts they describe — carrying no name, no address and
    // no message — which is the same behaviour the integrity audit has.
    await pool.query('DELETE FROM privacy_requests WHERE user_id = ANY($1)', [created.users]);
...
async function makeGiveaway(hostId, { deadlineDays = 7, status = 'active' } = {}) {
  ...
  await pool.query(
    `INSERT INTO giveaways (id, host_id, title, description, prize_description,
     ...
     VALUES ($1, $2, 'Rights giveaway', 'Fabricated', 'Fabricated prize', $3, $4, 'Self-funded')`,
...
test('14. the token is random, hashed, expiring and single-use', async () => {
  ...
  assert.ok(started.token.length >= 43, 'at least 256 bits of entropy, base64url encoded');
  assert.match(started.token, /^[A-Za-z0-9_-]+$/);
  ...
  const later = await inTransaction((client) =>
    rights.startEmailChange(client, { userId: user.id, newEmail: uniqueEmail('rights-expired') })
  );
  await pool.query(
...
test('15. the token never appears in the database in plaintext, in a response, or in a log', async () => {
  ...
  try {
    started = await session
      ...
      .set('X-Forwarded-For', nextTestIp())
      .send({ new_email: target, password: PASSWORD });
    // The send is fire-and-forget; give it a tick to reach the logger.
...
test('16. a failed send does not change the address', async () => {
  ...
  // A sender that throws is caught by the route and still leaves the address
  // alone — the route's .catch() is the only thing between the two.
  const routeSource = fs.readFileSync(
    path.join(__dirname, '..', 'server', 'routes', 'account.js'),
    'utf8'
  );
  assert.ok(
    /sendEmail\([\s\S]*?\}\)\.catch\(/.test(routeSource),
    'a send failure is caught rather than aborting or completing the change'
  );
  assert.ok(
    routeSource.indexOf("status: 'pending'") > routeSource.indexOf('sendEmail'),
    'the response says pending, never completed'
  );
});
...
test('17. two simultaneous completions produce exactly one change', async () => {
  ...
  const race = await inTransaction((client) =>
    rights.startEmailChange(client, { userId: user.id, newEmail: contested })
  );
...
test('21. the export excludes credentials, tokens, internal notes and detection signals', async () => {
  ...
  await inTransaction((client) =>
    rights.startEmailChange(client, { userId: entrant.id, newEmail: uniqueEmail('rights-exporttoken') })
  );
